...
import { isDefined, isFunction, isObject, newId } from '../util';
...
type ChildrenByGroupKeyMap = Map<string | HcPickOption, Array<HcPickOption>>;
...
export class ItemsList {
    ...
    get items(): Array<HcPickOption> { return this._items; }
    private _items = new Array<HcPickOption>();

    get filteredItems(): Array<HcPickOption> { return this._filteredItems; }
    private _filteredItems = new Array<HcPickOption>();

    private _optionGroups = new Array<HcPickOption>();
    ...
    get selectedItems(): Array<HcPickOption> { return this._selectionModel.value; }
    /** The HcOption that currently has focus */
    get markedItem(): HcPickOption { return this._filteredItems[this._markedIndex]; }
    /** The last option in the list to be highlighted */
    get lastSelectedItem(): HcPickOption | null {
     ...
     */
    findOption(value: any, favorBindValueStrategy: boolean = false): HcPickOption | undefined {
        let findBy: (item: HcPickOption) => boolean;
    ...
    addOption(option: HcPickOption) {
        ...
        if (parentGroup !== undefined) {
            ...
            const newParentGroup = new HcPickOption({
                groupKey: parentKey,
    ...
    removeOption(option: HcPickOption) {
    ...
    private _deleteItem(item: HcPickOption, list: Array<HcPickOption>) {
        const findIndexFunc = (i: HcPickOption) => i.index === item.index;
    ...
    private _groupItems(items: Array<HcPickOption>, groupBy: string | Function): Array<HcPickOption> {
    ...
    private _sortOptions(groups: Array<HcPickOption>) {
    ...
    */
    ...
    findByLabel(term: string): HcPickOption | undefined {
    ...
    filter(term: string): void {
        ...
        this._optionGroups.forEach(pg => {
            const matchedItems = new Array<HcPickOption>();
    ...
    createHcOption(item: any, index?: number): HcPickOption {
    ...
    private _defaultSearchFn(searchTerm: string, opt: HcPickOption): boolean {
        const label = opt.label?.toLocaleLowerCase();
        return label ? label.indexOf(searchTerm) > -1 : false;
    }
    ...
    private _getLastMarkedIndex(): number {
        ...
        const selectedIndex = this.lastSelectedItem ? this._filteredItems.indexOf(this.lastSelectedItem) : -1;
        if (this.lastSelectedItem && selectedIndex < 0) { return -1; }
        return Math.max(this.markedIndex, selectedIndex);
    }

    /** Group the items in the list as configured */
    private _groupBy(items: Array<HcPickOption>, groupBy: string | Function): ChildrenByGroupKeyMap {
        const groups: ChildrenByGroupKeyMap = new Map<string | HcPickOption, Array<HcPickOption>>();
    ...
    private _createParentHcOptions(groups: ChildrenByGroupKeyMap): Array<HcPickOption> {
        const isGroupByFn = isFunction(this._pickPane.groupBy);
        const parentOptions = new Array<HcPickOption>();
        for (const key of Array.from(groups.keys())) {
            const isObjectKey = isObject(key);
            const parent = new HcPickOption({
                groupKey: key,
            ...
            const groupValue = this._pickPane.groupValue || (() => {
                if (isObjectKey) { return (<HcPickOption>key).value; }
    ...
    private sortAndIndex(groups: Array<HcPickOption>): Array<HcPickOption> {
        const flattenedSortedItems = new Array<HcPickOption>();
