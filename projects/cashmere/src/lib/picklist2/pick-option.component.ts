...
export class HcPickOptionComponent implements OnChanges, AfterViewChecked, OnDestroy {
    @Input() value: any;
    @Input() set disabled(value: boolean) { this._disabled = parseBooleanAttribute(value); }
    get disabled() { return this._disabled; }
    get label(): string { return (this.elementRef.nativeElement.textContent || '').trim(); }

    readonly stateChange$ = new Subject<{ value: any, disabled: boolean, label?: string | null }>();
