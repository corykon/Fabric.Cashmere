...
import { HcPickPaneListService, PanelDimensions } from './pick-pane-list.service';
...
export class HcPickPaneListComponent implements OnInit, OnChanges, OnDestroy {
    @Input() items: HcPickOption[] = [];
    @Input() markedItem: HcPickOption;
    ...
    private get itemsLength() { return this._itemsLength; }
    ...
    scrollTo(option: HcPickOption, startFromOption = false) {
        if (!option) { return; }
        ...
        if (isDefined(scrollTo)) {
            this._scrollablePanel.scrollTop = scrollTo || 0;
        }
    }
    ...
    private _onItemsChange(items: HcPickOption[], firstChange: boolean) {
        this.items = items || [];
