...
import { isDefined, isFunction, isPromise, isObject, newId } from '../util';
import { ItemsList } from './items-list';
import { HcPickOption, SearchFn, TrackByFn } from './pick.types';
import { HcPickPaneListComponent } from './pick-pane-list.component';
import { SelectionModelFactory } from './selection-model';
import { HcPickPaneListService } from './pick-pane-list.service';
import { HcPicklist2Service } from './picklist2.service';
import { HcPickPaneDragService } from './pick-pane-drag.service';
...
export class HcPickPaneComponent implements OnDestroy, AfterViewInit, OnChanges {
    ...
    @HostBinding('class.hc-pick-pane')
    ...
    get selectedItems(): HcPickOption[] { return this.itemsList.selectedItems; }
    ...
    onSearchKeydown($event: KeyboardEvent) {
        if ($event.which === KEY_CODE.DOWN_ARROW || $event.which === KEY_CODE.ENTER) {
    ...
    private _selectAndScrollToItem(item: HcPickOption) {
        if (!item) { return; }
     ...
     */
    onItemClick($event: MouseEvent, item: HcPickOption) {
        if (!item) { return; }
    ...
    onItemDoubledClicked($event: MouseEvent, item: HcPickOption) {
    ...
    addAndSelectCustomOption() {
        let customItem;
        if (isFunction(this.addCustomItem) && this.searchTerm) {
    ...
    trackByOption = (_: number, item: HcPickOption) => {
    ...
    filter(term: string | null = this.searchTerm) {
        ...
        this.search.emit({ term: term || "", items: this.itemsList.filteredItems.map(x => x.value) });
    }
