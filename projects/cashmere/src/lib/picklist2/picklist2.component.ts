...
import { HcPicklist2Service } from './picklist2.service';
import { SortFn, GroupValueFn, CompareWithFn, AddCustomItemFn, SearchFn, HcPickOption, TrackByFn } from './pick.types';
...
export class HcPicklist2Component implements OnDestroy, AfterViewInit, ControlValueAccessor {
    ...
    @Output() change = new EventEmitter<Array<string|Object>>();
    /** Fires when options are added. */
    @Output() add = new EventEmitter<Array<string|Object>>();
    /** Fires when options are removed. */
    @Output() remove = new EventEmitter<Array<string|Object>>();
    ...
    /** 'Available' picklist pane */
    @ViewChild('available', { static: true }) leftPane: HcPickPaneComponent;
    /** 'Selected' picklist pane */
    @ViewChild('selected', { static: true }) rightPane: HcPickPaneComponent;
    ...
    ngAfterViewInit() {
        this.picklistService.reset(this.leftPane, this.rightPane);
        if (!this._itemsAreUsed) { this._setItemsFromNgOptions(); }
        if (isDefined(this.autoFocus)) { this.leftPane.focus(); }
        this._detectChanges();
    }
    ...
    writeValue(value: any | any[]): void {
        this.rightPane.itemsList.clearList();
    ...
    get hasValue(): boolean {
        return this.rightPane.itemsList.items.length > 0;
    }

    /** Returns true if the number of selected items matches or exceeds the maximum number */
    get hasMaxItemsSelected(): boolean {
        return Number.isFinite(this.maxSelectedItems) && this.rightPane.itemsList.items.length >= this.maxSelectedItems;
    }
    ...
    moveLeftToRight() {
        this._move(this.leftPane, this.rightPane, true);
    }

    /** Move highlighted items from the right pane over to the left pane. */
    moveRightToLeft() {
        this._move(this.rightPane, this.leftPane);
    }
    ...
    _move(source: HcPickPaneComponent, destination: HcPickPaneComponent, isAdding: boolean = false) {
        ...
        if (isAdding && this.maxSelectedItems) {
            overLimitBy = this.rightPane.itemsList.items.length + source.selectedItems.length - this.maxSelectedItems;
        ...
        this.leftPane.itemsList.clearSelected();
        this.rightPane.itemsList.clearSelected();
        this.refreshPanes();
        this._updateNgModel();
        this._onTouched();
    }
    ...
    public refreshScrollArea() {
        this.leftPane.refreshScrollArea();
        this.rightPane.refreshScrollArea();
    }
    ...
    _detectChanges() {
        ...
        this.leftPane.detectChanges();
        this.rightPane.detectChanges();
    }
    ...
    private _setItemsFromNgOptions() {
        const mapNgOptions = (options: QueryList<HcPickOptionComponent>) => {
            ...
            this.leftPane.itemsList.setItems(items);
            if (this.hasValue) {
        ...
        const handleOptionChange = () => {
            ...
            merge(...this.ngOptions.map(option => option.stateChange$))
                ...
                .subscribe(option => {
                    const item = this.leftPane.itemsList.findOption(option.value);
                    if (!item) { return; }
    ...
    private _handleWriteValue(ngModel: any[]) {
        ...
        const select = (val: any) => {
            let alreadySelected = this.rightPane.itemsList.findOption(val, true);
            if (alreadySelected) { return; }

            let item = this.leftPane.itemsList.findOption(val, true);
            if (item) {
                this.leftPane.itemsList.removeOption(item);
                this.rightPane.itemsList.addOption(item);
                this.leftPane.itemsList.reIndex();
            } else {
                const isValObject = isObject(val);
                const isPrimitive = !isValObject && !this.bindValue;
                if ((isValObject || isPrimitive)) {
                    this.rightPane.itemsList.addOption(this.rightPane.itemsList.createHcOption(val));
                    ...
                    this.rightPane.itemsList.addOption(this.rightPane.itemsList.createHcOption(rawitem));
                }
            }
        };
    ...
    private refreshPanes() {
        this.leftPane.filter();
        this.rightPane.filter();
        this._detectChanges();
    }
    ...
    private _updateNgModel() {
        ...
        const selectedItems = this.rightPane.itemsList.items.filter(i => !i.children);
        selectedItems.forEach(i => {
            let value = this.bindValue ? this.rightPane.itemsList.resolveNested(i.value, this.bindValue) : i.value;
            model.push(value);
        });
