...
export class UsageListComponent extends BaseDemoComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        ...
        this.httpClient.get<any>(environment.productCatalog.url + '/terms', options).subscribe({
            next: data => {
                data.forEach(term => {
    ...
    usageFilter(data: IUsage, filter: string): boolean {
        const catMatch =
            data.TermCategories.toLowerCase().includes(this.selectedCategoriesControl.value.toLowerCase()) ||
            this.selectedCategoriesControl.value === 'All';
        const typeMatch =
            data.TermTypes.toLowerCase().includes(this.selectedTypesControl.value.toLowerCase()) ||
            this.selectedTypesControl.value === 'All';
    ...
    submitRequest(): void {
        ...
        const postSub = this.httpClient
            .post<any>(
                environment.productCatalog.url + '/termchange.add',
                {
                    id: null,
                    submitDate: new Date(),
                    publishDate: null,
